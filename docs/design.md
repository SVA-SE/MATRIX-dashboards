...
Principles of visual design include tools such as emphasis, contrast, similarity, repetition, alignment, ... There appears to be as many lists of these principles as there are designers, with varying numbers, nomenclature and definition. A designer uses these tools to craft a specific experience for the viewer, by putting focus on specific elements, directing the eye along a specific path, and increasing visual interest and appeal. There are many wrong ways to do it, and no single right way, which is why the input of an experienced designer will always be beneficial.

If you don not feel confident in this task, **the safest option is always to keep things simple**. If you have a solid content structure from the previous step, that is already good enough. Most development tools will output a default product with a decent basic UI. Custom visual design will help creating a better product, but it is not strictly necessary.
